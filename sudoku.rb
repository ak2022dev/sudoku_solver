...
module Sudoku
  ...
  # - Rows and columns are numbered from 0 to 8, and the co-ordinates [0,0]
  #   refer to the cell in the upper-left corner of the puzzle.
  # - The nine 3x3 subgride are known as "boxes" and are also numbered from
  ...
  #
  # Create a new puzze with Sudoku::Puzzle.new, specifying the initial
  # stat as a string or as an array of string. The string(s) should use
  # the characters 1 through 9 for the given values, and '.' for cells
  ...
  #
  # The each_unknown is an iterator that loops through the cells of the
  # puzzle and invokes the associated block once for each cell whose
  # value is unknown.
  #
  # The possible method returns an arry of integers in the range 1..9.
  ...
  class Puzzle
    ...
    # This is the initialisation method for the class. It is automatically
    ...
    def initialize(lines)
      ...
      # Remove whitespeace (including newlines) from the data
    ...
    # This method overrides the Object.dup to copy the @grid array.
    def dup
    ...
    # and must be indexed with rown and column co-ordinates.
    def [](row, col)
      # Convert the two-dimensional (row,col) coordinates into a one-dimensional
    ...
    # capital letter, so this is a constant. Also, the array as been
    ...
    # Return ans array of all known values in the specified row.
    def rowdigits(row)
    ...
    # Map box number ot the index of the upper-left corner of the box.
  ...
  # set of possible values: there is no unknown cell in the puzzle that has fewer
  ...
  # or if the solve() method has made an incorrect guess.
  #
  # This method mutates the specified Puzzle object in place.
  # If has_duplicates? is falst on entry, then it will be false on exit.
  #
    ...
    until unchanged
      ...
      puzzle.each_unknown do |row, col, box|
        ...
        when 1  # We've found a unique value, so set it in the grid
          ...
          unchanged = false      # Note that we have made a change
  ...
  def Sudoku.solve(puzzle)
    ...
    # or an impossible puzzle. We'll know we have an impossible 
    ...
    p.each do |guess|      # For each value in the set of possible values
      ...
      begin
        ...
        next                  # If it raises and exception, try the next guess
      end
    end

    # If we get here, then one of our guesses worked out
